...
class Sumo_Yubikey_Model_Yubikey extends XenForo_Model
{
	// Input
	private $_id;
	...
	****************************************************************************/
	...
	/*
public function yubi_init($id, $signatureKey = null)
	{
		...
		//Get Yubikey ID and Key
		$this->_id = $options->sumo_yubikey_id;
		$this->_signatureKey = $options->sumo_yubikey_key;
		
		//Config overrides just in case
		$xf_cfg = XenForo_Application::get('config');
		if (isset($xf_cfg['yubikey']['id'])){
			$this->_id = $xf_cfg['yubikey_id'];
		}
		if (isset($xf_cfg['yubikey']['key'])){
			$this->_signatureKey = $xf_cfg['yubikey_key'];
		}
...
*/
	...
	public function verify($otp)
	{
		...
		$options = XenForo_Application::get('options');
		//Get Yubikey ID and Key
		$id = intval(trim($options->sumo_yubikey_id));
		$signatureKey = trim($options->sumo_yubikey_key);
		
		//Config overrides just in case
		$xf_cfg = XenForo_Application::get('config');
		if (isset($xf_cfg->yubikey->id)){
			$id = trim($xf_cfg->yubikey->id);
		}
		if (isset($xf_cfg->yubikey->key)){
			$signatureKey = trim($xf_cfg->yubikey->key);
		}
		...
		if ($this->_id == '' || $this->_signatureKey == ''){
			$this->_response = 'NO SIG OR ID';
			return false;
		}
